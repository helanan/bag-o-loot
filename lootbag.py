class LootBag():
    ...
    def write_toy_to_file(self, toy_name, child_id, action):
        with open('toylist', action) as toys:
            toy_id = uuid.uuid4()
            toy_list.write("{},{}\n".format(toy_id, toy_name, child_name))

    def add_to_bag(self, toy, child_name):
        """Add a toy and assign it to a child.  Both the toy and the child
        will be written to disk in the 'toylist' or 'children' file,
        respectively
        ...
        new_child_id = write_child_to_file(child_name, 'w')
        write_toy_to_file(toy, new_child_id, 'a')
    ...
    def remove_toy_from_child(self):
        pass
